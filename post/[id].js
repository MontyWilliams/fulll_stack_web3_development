...
export default function Post({ post }) {
  ...
  return (
    <div>
      {     //This lil operator "&&" cheks if posts is present then renders thge div
        post && (
...
export async function getStaticProps({ params }) {
  ...
  if(data.coverImage) {
    let coverImage = `${ipfsURI}/${data.coverImage}`    //cover image is called and then updated to include ipfs url
    data.coverImage = coverImage
  }

  return {
    props: {    //this is the prop thats past in at the begining of the app (async)
      post: data
    },
  }
}
